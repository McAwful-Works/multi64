-- Watch the M64P agent spliced into an Archipelago Ocarina of Time seed.
-- BizHawk: EmuHawk.exe --lua=agentprobe-oot.lua <spliced rom>. Writes agentprobe-oot.txt
-- next to itself and draws the numbers on screen.
--
-- Set AGENT_VRAM, AGENT_ROM and AGENT_MIN_RAM to profiles/oot/layout.env. The offsets are
-- from nm on agent/oot/build/agent.elf (multi64 with PEEKROM); re-read them if the agent changes.
local AGENT_VRAM = 0x80462000
local AGENT_ROM = 0x3582000
local AGENT_MIN_RAM = 0x800000 -- OoTR's payload needs the Expansion Pak anyway

local here = debug.getinfo(1, "S").source:match("^@(.*[/\\])") or ""
local out = here .. "agentprobe-oot.txt"
local base = AGENT_VRAM - 0x80000000

local ADDR = {
    magic         = base + 0x1CFC, -- gAgentSegmentMagic, 0x4D363450 once loaded
    init_attempts = base + 0x1D00, -- s_init_attempts (dormant for good at 16)
    ready         = base + 0x1D04, -- s_ready
    frames        = base + 0x1D08, -- s_frames_handled
    ticks         = base + 0x1D0C, -- s_ticks
    errors        = base + 0x5CEC, -- s_errors (mem_proto)
    requests      = base + 0x5CF8, -- s_requests (mem_proto)
}

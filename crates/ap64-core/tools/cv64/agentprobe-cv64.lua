-- Watch the M64P agent spliced into a Castlevania 64 seed.
-- BizHawk: EmuHawk.exe --lua=agentprobe-cv64.lua <spliced rom>. Writes agentprobe-cv64.txt
-- next to itself and draws the numbers on screen.
--
-- Set AGENT_VRAM, AGENT_ROM and AGENT_MIN_RAM to profiles/cv64/layout.env. The offsets are
-- from nm on agent/cv64/build/agent.elf (multi64 with PEEKROM); re-read them if the agent changes.
local AGENT_VRAM = 0x80480000
local AGENT_ROM = 0xC00000
local AGENT_MIN_RAM = 0x800000

local here = debug.getinfo(1, "S").source:match("^@(.*[/\\])") or ""
local out = here .. "agentprobe-cv64.txt"

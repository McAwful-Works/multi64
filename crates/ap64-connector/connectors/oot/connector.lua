--[[
AP64's fork of Archipelago's connector_oot.lua (see UPSTREAM), for a real N64 running the
M64P cart agent instead of BizHawk. Archipelago is MIT licensed; its notice is in UPSTREAM.

OoT Client talks to this exactly as it would to BizHawk: port 28921, one JSON line in and
one out. Everything that reads the game is upstream's, unchanged. What changed, and why:

* No emulator. The socket, the main loop, the BizHawk version check and common.lua are
  gone. AP64 owns the TCP port and calls handle() with each line, sending back what it
  returns. print goes to AP64's log.

* Memory. `mainmemory` is cartmem.lua: RDRAM through the cart, served from a page cache
...
Provided by AP64 (crates/ap64-connector): the global `ap64`, and json, bit, cartmem via require.
]]
...
function process_block(block)
    ...
    -- AP64: the count and the mailbox (player and item) from one instant. See the header.
    cartmem.refresh({ { internal_count_addr, 2 }, { incoming_player_addr, 4 } })
...
-- Main control handling. AP64: one line from OoT Client in, one out (see the header).

function handle(line)
    cartmem.begin_poll()
    process_block(json.decode(line))

//! Send a short **raw L3 octet** payload through [`Sc64L2Pipe`] and compare with the echoed bytes (no `M64B` framing).
//!
//! Run **`multi64_test.z64`** in **RAW_ECHO** mode (default), then: `cargo run -p sc64-echo-test -- --port COM3`
...
struct Args {
    ...
    /// Payload to send (must fit in one outbound L2 chunk for this test; default ≤ 8192 bytes — reference SC64 `USB_WRITE` chunk size)

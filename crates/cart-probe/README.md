...
2. **Identity probes** (`probe_port`). Sends SC64 `IDENTIFIER_GET`, then the PRO's edlink handshake at 921600 baud, then the X-series `usb64` test. The device on the other end receives those bytes. A port held by another process cannot be opened and reports nothing. A port with no cart costs about three seconds. `probe_port_cancellable` takes a cancel check and stops at its next read, within about 100 ms, once it returns true; Multi64 passes its exit flag.

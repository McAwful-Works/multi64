...
Styling is tokenised: `:root` in each app's `styles.css` holds the palette and **no rule outside it may contain a colour literal**, or the change survives unchanged into the Light and High-contrast themes. `appearance.js` is duplicated verbatim in both apps, must stay Tauri-free, and must load non-deferred in `<head>`. `styles.css` is duplicated verbatim too: it is the shared base (palette, size tokens, buttons, fields, dialogs), app-only rules go in the app's own sheet, and a test fails if the two copies differ. See [`docs/frontend-appearance.md`](docs/frontend-appearance.md) before editing any CSS or frontend JS.

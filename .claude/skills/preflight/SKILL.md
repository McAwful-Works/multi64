---
name: preflight
description: Run the six steps of CI's Rust job (cargo fmt, the multi64d builds, clippy with -D warnings, test, release build) in order and report the first failure with enough context to act on it. Use before handing work back, before opening a PR, or whenever asked whether a change will pass CI.
---

# Pre-flight: the checks CI actually runs

`.github/workflows/ci.yml`'s Rust job runs exactly these six steps, in this order, on `ubuntu-latest` and `windows-latest`. No CI job touches hardware.

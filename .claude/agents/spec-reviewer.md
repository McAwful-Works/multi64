...
7. **`Ed64L2Pipe` implements `l3-over-everdrive-x7.md` §4** but has run on only one X7, which could not send while receiving (§4.5 item 6), and the spec stays **Draft** until §4.5 is answered on hardware. Flag anything that presents the mapping as validated — dropping Draft, deleting §4.5, or describing the ED64 e2e tools as proven — without a hardware result to back it.

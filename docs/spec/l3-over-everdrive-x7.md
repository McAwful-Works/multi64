...
6. **Sending while receiving. Observed on one X7: the cart's own writes give up.** With the test ROM echoing each host message as it arrives, a burst of 17 messages came back with one cut off after its first 512-byte block; the host found the next message's payload where `CMPH` belonged. The same 17 messages sent one at a time, each echo read before the next, came back intact. With test ROM 1.11, which counts writes that give up (`DIAG` `tx_failures`, `test-l3-application-v0.md` §11), the count rose by **6** across that burst and by none elsewhere in the run: the messages are lost on the cart, not on the cable or in the host. libdragon's `usb_everdrive_write` gives up when the USB unit stays busy for 100 ms and returns part-way through the message; `n64/agent/ed64.c`'s `ed64_send` has the same shape. The host already survives it: §4.4's resynchronisation drops the broken message and the stream continues, so the cost is a lost message, not a lost link.

   **Still open: why the USB unit stays busy.** libdragon's own comment on that write says it will not write while there is data to read. If unread host data blocks a write, waiting longer cannot help and the cart must read first; if the unit is only slow, a longer wait that finishes the message is enough. The two call for different fixes to `ed64_send`, and the wrong one could hang the cart. Test ROM 1.12 reads everything the host has already sent before each echo; a burst that then passes with `tx_failures` unchanged points to the first.

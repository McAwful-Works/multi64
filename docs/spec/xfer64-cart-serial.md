...
| [`cart_probe.rs`](../../crates/xfer64/src-tauri/src/cart_probe.rs) | Auto-detect over [`multi64-cart-probe`](../../crates/cart-probe/README.md): an SC64 by its USB descriptors first (no writes, so it works while `multi64d` holds the port), then SC64 `IDENTIFIER_GET`, the EverDrive-64 PRO handshake at **921600**, and EverDrive **`usb64`** `cmd`/`t` at **115200**. The `multi64d` yield check uses only a pinned port and never probes. Before any of that, Auto takes the port and cart a running `multi64d` reports (`GET /` `serial` and `cart`, [daemon API §1.1](./daemon-api-v1.md)) while that port is still plugged in. |
